...
function createPgDatabase(dbConfig, overrides) {
...
function buildPgDatabaseConfig(dbConfig, overrides) {
  ...
  return mergeDatabaseConfig(dbConfig.get(), {
    connection: overrides && (typeof connection === "string" || connection instanceof String) ? parsePgConnectionString(connection) : connection,
...
function createSqlite3Database(dbConfig, overrides) {
...
function buildSqlite3DatabaseConfig(dbConfig, overrides) {
  return mergeDatabaseConfig(dbConfig.get(), {
    useNullAsDefault: true
  }, overrides);
}

function createDatabase(config3, overrides) {
  const client = config3.getString("client");
  if (client === "pg") {
    return createPgDatabase(config3, overrides);
  } else if (client === "sqlite3") {
    return createSqlite3Database(config3);
  }
  return knex2(mergeDatabaseConfig(config3.get(), overrides));
}
